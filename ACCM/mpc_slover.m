...
function [v,J_sum,J,u_to,u_toN] = mpc_slover(N,col,u_to,u_toN,V_vir,L)
...
v = fmincon(@(v)myfun(V_vir,L,v),v0,[],[],Aeq,beq,lb,ub,nonlcon,options);
...
J_sum = -myfun(V_vir,L,v); % f: a const, all current coalition fish for N times
