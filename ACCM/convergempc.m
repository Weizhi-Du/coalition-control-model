...
function [u_toN,vto_con,Jto_sumc,J_sum,J_hat] = convergempc(u_toc,col_total,x,xeb_con,max_it,V_vir,L)
%
% Input
...
for t_d = 1:1:max_it
    ...
    for t_c = 1:1:size(col_total,1)
        ...
        [v,J_sum,~,u_to,u_toN]  = mpc_slover(N,col,u_to,u_toN,V_vir,L);
    ...
    for t_hat = 1:1:Num_boat
        ...
        J_hat(t_hat,t_d) = -cost_hat(v_hat,u_toN,Gam{t_hat},B_all);
    end
    vto_con(:,t_d) = v_con;
    Jto_sumc(:,t_d) = J_sumc;
%     vto_con(:,t_d) = [0];
